...
async function setup_comparison_builds(commits: string[]) {
	...
	for (let i = 0; i < commits.length; i++) {
		...
		await execFileAsync('git', ['worktree', 'add', temp_dir, commit], { stdio: 'inherit' })
		temp_dirs.push(temp_dir)

		// Build in the worktree
		await execFileAsync('npm', ['install'], { stdio: 'inherit', cwd: temp_dir })
		await execFileAsync('npm', ['run', 'build'], { stdio: 'inherit', cwd: temp_dir })
	...
	const cleanup = async () => {
		...
		for (const temp_dir of temp_dirs) {
			try {
				await execFileAsync('git', ['worktree', 'remove', temp_dir], { stdio: 'inherit' })
			} catch (error) {
