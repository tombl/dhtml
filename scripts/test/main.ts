...
async function setup_comparison_builds(commits: string[]) {
	...
	const buildPromises = commits.map(async commit => {
		...
		await execFileAsync('git', ['worktree', 'add', temp_dir, commit], { stdio: 'inherit' } as ExecFileOptions)

		// Install dependencies and build
		await execFileAsync('npm', ['install'], { stdio: 'inherit', cwd: temp_dir } as ExecFileOptions)
		await execFileAsync('npm', ['run', 'build'], { stdio: 'inherit', cwd: temp_dir } as ExecFileOptions)
	...
	const cleanup = async () => {
		...
		await Promise.all(
			temp_dirs.map(async temp_dir => {
				try {
					await execFileAsync('git', ['worktree', 'remove', temp_dir], { stdio: 'inherit' } as ExecFileOptions)
				} catch (error) {
