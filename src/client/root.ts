...
import { append_child, get_attribute, remove_attribute } from './dom.ts'
...
export function create_root(span: Span): Root {
	...
	let parts: { _i: number; _part: Part }[] | undefined

	function detach() {
		if (!parts) return
		// scan through all the parts of the previous tree, and clear any renderables.
		for (let i = 0; i < parts.length; i++) parts[i]._part(null)
		parts = undefined
	}
		...
		render: (value: Displayable) => {
			...
			if (old_template !== template) {
				...
				for (const node of doc.querySelectorAll('[data-dynparts]')) {
					const parts = get_attribute(node, 'data-dynparts')
					assert(parts)
					remove_attribute(node, 'data-dynparts')
				...
				parts = template._parts.map(([dynamic_index, createPart], element_index) => ({
					_i: dynamic_index,
					_part: createPart(node_by_part[element_index], span),
				}))
			}
